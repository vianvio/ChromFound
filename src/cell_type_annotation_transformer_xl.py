...
from src.models.transformer_xl_genomic import FinetuneGenomicTransformerXLCellType
...
def main_finetune():
    ...
    # Update model args to use Transformer-XL specific parameters
    pretrain_model_args["n_head"] = 8  # Number of attention heads
    pretrain_model_args["mem_len"] = 512  # Memory length for Transformer-XL
    pretrain_model_args["max_pos_len"] = 100000  # Maximum genomic position length
    ...
    # Use the Transformer-XL model instead of the Mamba model
    model = FinetuneGenomicTransformerXLCellType(**pretrain_model_args)
    model = model.to(device)
