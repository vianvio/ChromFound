...
from src.lora import add_lora_to_model, get_lora_parameters, freeze_non_lora_parameters
...
def cell_type_finetune(
    ...
    best_f1_score = 0.0
    patience_counter = 0
    patience = finetune_args.get("patience", 5)  # Default to 5 epochs without improvement
    ...
    for eph in range(finetune_args.get("epoch")):
        ...
        for batch in train_dataloader:
            ...
            # Gradient clipping
            if finetune_args.get("gradient_clipping", False):
                torch.nn.utils.clip_grad_norm_(model.parameters(), finetune_args.get("max_grad_norm", 1.0))
...
def main_finetune():
    ...
    parser.add_argument("--lora_alpha", type=float, default=32.0, help="alpha parameter for LoRA")
    ...
    parser.add_argument("--backbone_lr", type=float, default=1e-5, help="learning rate for backbone network")
    parser.add_argument("--classifier_lr", type=float, default=1e-3, help="learning rate for classifier head")
    ...
    if args.use_lora:
        finetune_logger.info(f'Applying LoRA with rank={args.lora_rank}, alpha={args.lora_alpha}')
        model = add_lora_to_model(model, rank=args.lora_rank, alpha=args.lora_alpha, dropout=0.1)
        freeze_non_lora_parameters(model)
        # Only optimize LoRA parameters
        optimizer_params = [
            {"params": get_lora_parameters(model), "lr": args.learning_rate, "weight_decay": 1e-6}
        ]
        finetune_logger.info(f'Using LoRA - only {len(get_lora_parameters(model))} parameters will be trained')
    else:
        # Hierarchical learning rate strategy
        ...
        for name, param in model.named_parameters():
            ...
            elif param.requires_grad:  # Only include params that are trainable
                backbone_params.append(param)
        
        optimizer_params = [
            {"params": backbone_params, "lr": args.backbone_lr, "weight_decay": 1e-6},
            {"params": classifier_params, "lr": args.classifier_lr, "weight_decay": 1e-6}
        ]
        
        finetune_logger.info(f'Using hierarchical learning rates: backbone={args.backbone_lr}, classifier={args.classifier_lr}')

    optimizer = torch.optim.AdamW(optimizer_params, betas=(0.8, 0.999), eps=1e-8)
