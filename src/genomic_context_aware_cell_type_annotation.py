...
from src.models.genomic_context_contrastive import (
    GenomicProximitySampler, 
    CellTypeSpecificAnchor, 
    GenomicContextAwareContrastiveLoss, 
    GenomicAttention,
    RegionalLevelContrastive
)
...
class GenomicContextAwareFinetuneModelMambaCellType(PretrainModelMambaLM):
    def __init__(self, **kwargs):
        ...
        # Add genomic context-aware components
        self.genomic_proximity_sampler = GenomicProximitySampler(proximity_threshold=kwargs.get("proximity_threshold", 1000000))
        self.cell_type_specific_anchor = CellTypeSpecificAnchor(
            embedding_dim=self.model_args["embedding_dim"],
            num_cell_types=self.model_args["cell_type_num"],
            num_anchors=kwargs.get("num_anchors", 10)
        )
        self.genomic_attention = GenomicAttention(
            embed_dim=self.model_args["embedding_dim"],
            num_heads=kwargs.get("attention_heads", 8)
        )
        self.regional_contrastive = RegionalLevelContrastive(
            region_size=kwargs.get("region_size", 50),
            embedding_dim=self.model_args["embedding_dim"]
        )
        
        # Contrastive loss components
        self.contrastive_loss = GenomicContextAwareContrastiveLoss(
            temperature=kwargs.get("contrastive_temperature", 0.07),
            genomic_weight=kwargs.get("genomic_weight", 0.5),
            regional_weight=kwargs.get("regional_weight", 0.3)
        )
        
        # Contrastive projection head
        self.contrastive_projection = torch.nn.Sequential(
            torch.nn.Linear(self.model_args["embedding_dim"], 512),
            torch.nn.BatchNorm1d(512),
            torch.nn.ReLU(),
            torch.nn.Linear(512, 128)
        )

    def forward(self, value, chromosome, hg38_start, hg38_end, return_contrastive_features=False):
        x = self.embedding(value, chromosome.long(), hg38_start.long(), hg38_end.long())
        x = self.backbone(x)
        
        # Apply genomic attention
        genomic_positions = torch.stack([hg38_start, hg38_end], dim=-1)  # [batch, seq_len, 2]
        x_attended, attn_weights = self.genomic_attention(x, genomic_positions)
        
        # Feature projection for cell type prediction
        x_projected = self.feature_projection(x_attended)
        x_projected = torch.squeeze(x_projected, dim=-1)
        x_projected = self.post_backbone_dropout(x_projected)
        x_cell_type_prediction = self.ft_cell_type_projection(x_projected)
        
        if return_contrastive_features:
            # Extract contrastive features
            contrastive_features = self.contrastive_projection(x_attended.mean(dim=1))  # Global pooling
            regional_features = self.regional_contrastive(x_attended)
            return x_cell_type_prediction, contrastive_features, regional_features, attn_weights
        
        return x_cell_type_prediction

    def compute_contrastive_loss(self, contrastive_features, regional_features, 
                                cell_types, chromosomes, starts, ends):
        """Compute the genomic context-aware contrastive loss"""
        # Generate genomic adjacency matrix
        genomic_adjacency = self.genomic_proximity_sampler.sample_positive_pairs(
            chromosomes, starts, ends
        )
        
        # Compute contrastive loss at global level
        contrastive_loss_global = self.contrastive_loss(
            contrastive_features, cell_types, genomic_adjacency
        )
        
        # Compute contrastive loss at regional level
        batch_size, num_regions, regional_dim = regional_features.shape
        regional_flat = regional_features.view(-1, regional_dim)
        cell_types_expanded = cell_types.unsqueeze(1).expand(-1, num_regions).contiguous().view(-1)
        contrastive_loss_regional = self.contrastive_loss(
            regional_flat, cell_types_expanded
        )
        
        # Combine losses
        total_contrastive_loss = contrastive_loss_global + contrastive_loss_regional
...
def cell_type_finetune(
    ...
    step = 0
    best_f1_score = 0.0
    contrastive_weight = finetune_args.get("contrastive_weight", 0.1)  # Weight for contrastive loss
    ...
    for eph in range(finetune_args.get("epoch")):
        for batch in train_dataloader:
            ...
            cell_type_output, contrastive_features, regional_features, attn_weights = model(
                value, chromosome, pos_start, pos_end, return_contrastive_features=True
            )
            
            # Compute cell type prediction loss
            ...
            contrastive_loss = model.compute_contrastive_loss(
                contrastive_features, regional_features, 
                cell_type, chromosome, pos_start, pos_end
            )
            
            # Combined loss
            total_loss = loss_cell_type_prediction + contrastive_weight * contrastive_loss
            ...
            if step % finetune_args.get("loss_evaluate", 10) == 0:
                ...
                logger.info(
                    ...
                    f"(cell_type: {loss_cell_type_prediction.item():.4f}, "
...
def main_finetune():
    ...
    parser.add_argument("--proximity_threshold", type=int, default=1000000, help="threshold for genomic proximity (bp)")
    parser.add_argument("--num_anchors", type=int, default=10, help="number of cell type specific anchors")
    parser.add_argument("--attention_heads", type=int, default=8, help="number of attention heads")
    parser.add_argument("--region_size", type=int, default=50, help="size of genomic regions for regional contrastive learning")
    parser.add_argument("--contrastive_temperature", type=float, default=0.07, help="temperature for contrastive loss")
    parser.add_argument("--genomic_weight", type=float, default=0.5, help="weight for genomic context in contrastive loss")
    parser.add_argument("--regional_weight", type=float, default=0.3, help="weight for regional contrastive loss")
    ...
    # Add genomic context-aware parameters
    pretrain_model_args["proximity_threshold"] = args.proximity_threshold
    pretrain_model_args["num_anchors"] = args.num_anchors
    pretrain_model_args["attention_heads"] = args.attention_heads
    pretrain_model_args["region_size"] = args.region_size
    pretrain_model_args["contrastive_temperature"] = args.contrastive_temperature
    pretrain_model_args["genomic_weight"] = args.genomic_weight
    pretrain_model_args["regional_weight"] = args.regional_weight
    pretrain_model_args["contrastive_weight"] = args.contrastive_weight
    ...
    model = GenomicContextAwareFinetuneModelMambaCellType(**pretrain_model_args)
    model = model.to(device)
