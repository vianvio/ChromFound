...
from src.models.genomic_multi_scale_attention import MultiScaleGenomicModel
...
class FinetuneModelMultiScaleCellType(torch.nn.Module):
    def __init__(self, **kwargs):
        super().__init__()
        self.model_args = kwargs
        max_length = self.model_args["max_length"]

        # Use the original embedding layer
        ...
        # Use the multi-scale genomic model for the backbone
        self.multiscale_backbone = MultiScaleGenomicModel(
            embedding_dim=self.model_args["embedding_dim"],
            num_classes=self.model_args["cell_type_num"],
            max_length=max_length,
            encoder_layers=self.model_args.get("encoder_layers", 64),
            kernel_sizes=[3, 7, 15, 31],  # Different genomic scales
            num_genomic_contexts=4  # Promoter, gene body, enhancer, intergenic
        )

        # Freeze the original mask token prediction if needed
        self.mask_token_prediction = torch.nn.Linear(self.model_args["embedding_dim"], 1)  # Placeholder
        for param in self.mask_token_prediction.parameters():
            param.requires_grad = False

    def forward(self, value, chromosome, hg38_start, hg38_end, **kwargs):
        # First, get the embedded representation using the original embedding
        x = self.embedding(value, chromosome.long(), hg38_start.long(), hg38_end.long())

        # Then pass through the multi-scale genomic model
        x_cell_type_prediction = self.multiscale_backbone(x)
...
def main_finetune():
    ...
    model = FinetuneModelMultiScaleCellType(**pretrain_model_args)
    model = model.to(device)
    ...
    if args.load_pretrain_ckpt:
        ...
        # Load only compatible parts of the state dict (the embedding layer)
        model_dict = model.state_dict()

        # Filter out keys that match the embedding layer
        embedding_keys = [k for k in model_dict.keys() if k.startswith('embedding.')]
        pretrained_dict = {}

        for k, v in state_dict['module'].items():
            # Map the original keys to the new embedding keys
            if k.startswith('embedding.'):
                new_k = k.replace('embedding.', 'embedding.')
                if new_k in model_dict:
                    pretrained_dict[new_k] = v
            # Also map backbone weights if they match
            elif k.startswith('backbone.'):
                # We won't load backbone weights since we're using a different architecture
                continue
            elif k.startswith('mask_token_prediction.'):
                # We won't load mask token prediction since we have a placeholder
                continue

        # Update existing keys
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict, strict=False)
        print(f"Loaded pre-trained weights for compatible layers. Loaded {len(pretrained_dict)} keys.")
