...
class HierarchicalContrastiveLoss(torch.nn.Module):
    """
    Hierarchical Contrastive Learning Framework that incorporates cell type ontology relationships
    """
    def __init__(self, temperature=0.07, hard_negative_weight=0.5, hierarchy_levels=None):
        super(HierarchicalContrastiveLoss, self).__init__()
        self.temperature = temperature
        self.hard_negative_weight = hard_negative_weight
        self.hierarchy_levels = hierarchy_levels or {}  # Maps cell types to their hierarchy levels
        
    def forward(self, embeddings, labels, hierarchy_similarities=None):
        """
        Args:
            embeddings: [batch_size, embedding_dim]
            labels: [batch_size] - cell type indices
            hierarchy_similarities: [num_cell_types, num_cell_types] - similarity matrix based on ontology
        """
        ...
        # Create label-based mask for positive pairs
        labels_expanded = labels.unsqueeze(1)
        positive_mask = (labels_expanded == labels_expanded.T) & (~mask)
        
        # If hierarchy similarities are provided, adjust the loss based on biological relationships
        if hierarchy_similarities is not None:
            # Map labels to their hierarchy-based similarities
            label_indices = labels.cpu().numpy()
            hierarchy_similarity_matrix = hierarchy_similarities[label_indices][:, label_indices].to(device)
            
            # Adjust similarity matrix based on hierarchy
            adjusted_similarity_matrix = similarity_matrix + hierarchy_similarity_matrix
            
            # Positive pairs based on hierarchy (not just identical labels)
            # Cells of similar types according to hierarchy should also be treated as positives
            hierarchy_threshold = 0.5  # Threshold for considering cells as hierarchically similar
            hierarchy_positive_mask = (hierarchy_similarity_matrix > hierarchy_threshold) & (~mask)
            positive_mask = positive_mask | hierarchy_positive_mask
        else:
            adjusted_similarity_matrix = similarity_matrix
        
        # Calculate contrastive loss
        exp_similarities = torch.exp(adjusted_similarity_matrix)
        
        # Sum of positive similarities for each sample
        pos_similarities = torch.sum(exp_similarities * positive_mask.float(), dim=1)
        
        # Sum of all similarities for each sample (denominator)
        all_similarities = torch.sum(exp_similarities, dim=1)
        
        # Contrastive loss
        contrastive_loss = -torch.log(pos_similarities / all_similarities + 1e-8)

        return contrastive_loss.mean()


class MultiLevelHierarchicalContrastiveLoss(torch.nn.Module):
    """
    Multi-Level Hierarchical Contrastive Loss that considers multiple levels of cell type similarity
    based on biological ontologies (e.g., immune cells vs. epithelial cells)
    """
    def __init__(self, temperature=0.07, intra_level_weight=1.0, inter_level_weight=0.5):
        super(MultiLevelHierarchicalContrastiveLoss, self).__init__()
        self.temperature = temperature
        self.intra_level_weight = intra_level_weight  # Weight for same cell type contrastive loss
        self.inter_level_weight = inter_level_weight  # Weight for similar cell type contrastive loss

    def forward(self, embeddings, labels, cell_type_hierarchy=None):
        """
        Args:
            embeddings: [batch_size, embedding_dim] - normalized embeddings
            labels: [batch_size] - cell type indices
            cell_type_hierarchy: [num_cell_types, num_cell_types] - hierarchy relationship matrix
        """
        device = embeddings.device
        batch_size = embeddings.shape[0]
        ...
        # Create masks for different levels of similarity
        labels_expanded = labels.unsqueeze(1)
        same_type_mask = (labels_expanded == labels_expanded.T) & (~mask)  # Same cell type

        # If hierarchy is provided, create similar type mask
        if cell_type_hierarchy is not None:
            label_indices = labels.cpu().numpy()
            hierarchy_matrix = cell_type_hierarchy[label_indices][:, label_indices].to(device)
            # Similar but not identical cell types based on hierarchy
            hierarchy_threshold = 0.3  # Lower threshold for similar types
            similar_type_mask = (hierarchy_matrix > hierarchy_threshold) & (~same_type_mask) & (~mask)
        else:
            similar_type_mask = torch.zeros_like(similarity_matrix, dtype=torch.bool)

        # Calculate different components of the loss
        # 1. Intra-level loss (same cell types should be close)
        same_type_similarities = torch.exp(similarity_matrix) * same_type_mask.float()
        same_pos_similarities = torch.sum(same_type_similarities, dim=1)
        all_similarities = torch.sum(torch.exp(similarity_matrix), dim=1)
        intra_loss = -torch.log(same_pos_similarities / all_similarities + 1e-8)

        # 2. Inter-level loss (similar but different cell types should be moderately close)
        if cell_type_hierarchy is not None:
            similar_type_similarities = torch.exp(similarity_matrix) * similar_type_mask.float()
            similar_pos_similarities = torch.sum(similar_type_similarities, dim=1)
            inter_loss = -torch.log(similar_pos_similarities / all_similarities + 1e-8)
            inter_loss = inter_loss[similar_pos_similarities > 0].mean() if torch.any(similar_pos_similarities > 0) else torch.tensor(0.0, device=device)
        else:
            inter_loss = torch.tensor(0.0, device=device)

        # Combined loss
        total_loss = self.intra_level_weight * intra_loss.mean() + self.inter_level_weight * inter_loss
...
class HardNegativeMining:
    """
    Hard Negative Mining to focus on difficult-to-distinguish cell types
    """
    def __init__(self, mining_strategy='semi_hard'):
        """
        Args:
            mining_strategy: 'all', 'hard', 'semi_hard' - strategy for selecting hard negatives
        """
        self.mining_strategy = mining_strategy

    def mine_hard_negatives(self, embeddings, labels, margin=0.1):
        """
        Identifies hard negatives based on distance in embedding space
        """
        device = embeddings.device
        batch_size = embeddings.shape[0]
        ...
        # Compute cosine similarity (more appropriate for normalized embeddings)
        similarity_matrix = torch.matmul(embeddings, embeddings.T)
        # Convert to distance
        distance_matrix = 2.0 - 2.0 * similarity_matrix  # ||a-b||^2 = 2 - 2*cos(a,b) for unit vectors

        # Create mask for different labels
        labels_expanded = labels.unsqueeze(1)
        different_labels_mask = (labels_expanded != labels_expanded.T)

        # Create mask for same labels
        same_labels_mask = (labels_expanded == labels_expanded.T)

        if self.mining_strategy == 'all':
            # All negatives regardless of distance
            hard_negative_mask = different_labels_mask
        elif self.mining_strategy == 'hard':
            # Only negatives that are closer than margin
            hard_negative_mask = (distance_matrix < margin) & different_labels_mask
        elif self.mining_strategy == 'semi_hard':
            # Negatives that are closer than positives
            # For each anchor, find the hardest positive and softest negative
            hard_negative_mask = torch.zeros_like(distance_matrix, dtype=torch.bool)
            for i in range(batch_size):
                anchor_label = labels[i]
                positive_mask = same_labels_mask[i]  # Positives for this anchor
                negative_mask = different_labels_mask[i]  # Negatives for this anchor

                if positive_mask.sum() > 0 and negative_mask.sum() > 0:
                    # Find max distance among positives (hardest positive)
                    pos_distances = distance_matrix[i][positive_mask]
                    hardest_positive_dist = pos_distances.max()

                    # Select negatives that are closer than the hardest positive
                    neg_distances = distance_matrix[i]
                    semi_hard_negatives = (neg_distances < hardest_positive_dist) & negative_mask
                    hard_negative_mask[i] = semi_hard_negatives
        else:
            # Default to semi-hard mining
            hard_negative_mask = torch.zeros_like(distance_matrix, dtype=torch.bool)
            for i in range(batch_size):
                anchor_label = labels[i]
                positive_mask = same_labels_mask[i]  # Positives for this anchor
...
class ContrastiveFinetuneModelMambaCellType(PretrainModelMambaLM):
    def __init__(self, **kwargs):
        ...
        # Contrastive projection head for embedding space
    ...
    def forward(self, value, chromosome, hg38_start, hg38_end, return_contrastive=False):
        ...
        if return_contrastive:
            ...
            x_contrastive = self.contrastive_projection(x)
            return x_cell_type_prediction, x_contrastive
...
def cell_type_finetune_with_contrastive(
    ...
    contrastive_criterion = MultiLevelHierarchicalContrastiveLoss(temperature=0.07, intra_level_weight=1.0, inter_level_weight=0.5)
    hard_negative_miner = HardNegativeMining(mining_strategy='semi_hard')

    step = 0
    best_f1_score = 0.0

    # Create a placeholder for cell type hierarchy (in practice, this would come from biological ontologies)
    # For now, we'll create a simple hierarchy based on common cell type relationships
    num_cell_types = model.model_args["cell_type_num"]
    cell_type_hierarchy = torch.ones(num_cell_types, num_cell_types, device=device) * 0.1  # Default low similarity
    # Set diagonal to 1.0 (perfect similarity with self)
    cell_type_hierarchy.fill_diagonal_(1.0)
    ...
    for eph in range(finetune_args.get("epoch")):
        for batch in train_dataloader:
            ...
            # Forward pass with contrastive embeddings
            cell_type_output, contrastive_embeddings = model(
                value, chromosome, pos_start, pos_end, return_contrastive=True
            )

            # Compute Focal Loss for cell type classification
            loss_cell_type_prediction = cell_type_criterion(cell_type_output, cell_type)

            # Compute Multi-Level Hierarchical Contrastive Loss
            loss_contrastive = contrastive_criterion(contrastive_embeddings, cell_type, cell_type_hierarchy)

            # Apply hard negative mining
            hard_negative_mask = hard_negative_miner.mine_hard_negatives(contrastive_embeddings, cell_type, margin=0.5)
            if hard_negative_mask.any():
                # Increase weight for hard negatives
                hard_negative_factor = 2.0
                loss_contrastive = loss_contrastive * hard_negative_factor

            # Combined loss
            total_loss = loss_cell_type_prediction + 0.1 * loss_contrastive  # Weight for contrastive loss
            ...
            if step % finetune_args.get("loss_evaluate", 10) == 0:
                ...
                logger.info(
                    ...
                    f"classification loss: {loss_cell_type_prediction.item():.4f}, "
...
def main_finetune():
    ...
    model = ContrastiveFinetuneModelMambaCellType(**pretrain_model_args)
    model = model.to(device)
    ...
    cell_type_finetune_with_contrastive(
        model,
        finetune_config,
        train_dataloader,
        val_dataloader,
