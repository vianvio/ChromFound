...
def freeze_all_backbone_layers(model):
    """Freeze all backbone layers"""
    for param in model.backbone.parameters():
        param.requires_grad = False


def unfreeze_last_n_blocks(model, n_blocks_to_unfreeze):
    """Unfreeze the last n blocks of the backbone"""
    total_blocks = len(model.backbone.layers)
    ...
    start_idx = max(0, total_blocks - n_blocks_to_unfreeze)
...
def cell_type_finetune_progressive(
        model,
        finetune_args,
        train_dataloader,
        val_dataloader,
    ...
    unfreezing_schedule = [
        {"epochs": 5, "blocks_to_unfreeze": 1, "lr": finetune_args.get("learning_rate")},
        {"epochs": 3, "blocks_to_unfreeze": 2, "lr": finetune_args.get("learning_rate") * 0.8},
        {"epochs": 3, "blocks_to_unfreeze": 4, "lr": finetune_args.get("learning_rate") * 0.6},
        {"epochs": 3, "blocks_to_unfreeze": 8, "lr": finetune_args.get("learning_rate") * 0.5},
        {"epochs": 3, "blocks_to_unfreeze": -1, "lr": finetune_args.get("learning_rate") * 0.3},  # Unfreeze all remaining
    ]
    
    step = 0
    best_f1_score = 0.0
    
    for stage_idx, stage in enumerate(unfreezing_schedule):
        epochs = stage["epochs"]
        blocks_to_unfreeze = stage["blocks_to_unfreeze"]
        lr = stage["lr"]
        
        # Freeze all backbone layers initially
        freeze_all_backbone_layers(model)
        
        # Unfreeze the specified number of blocks
        if blocks_to_unfreeze == -1:  # Unfreeze all
            for param in model.backbone.parameters():
                param.requires_grad = True
            logger.info(f"Stage {stage_idx + 1}: Unfreezing ALL backbone layers")
        else:
            unfreeze_last_n_blocks(model, blocks_to_unfreeze)
            logger.info(f"Stage {stage_idx + 1}: Unfreezing last {blocks_to_unfreeze} blocks")
        
        # Only unfreeze the classification head
        for param in model.ft_cell_type_projection.parameters():
            param.requires_grad = True
        ...
        logger.info(f"Stage {stage_idx + 1}: Trainable parameters: {trainable_params:,}")
        ...
        logger.info(f"Starting stage {stage_idx + 1} with {epochs} epochs, LR: {lr}")
        ...
        for eph in range(epochs):
            for batch in train_dataloader:
                ...
                if step % finetune_args.get("loss_evaluate", 10) == 0:
                    ...
                    logger.info(
                        f"[Train] Stage {stage_idx + 1} Epoch {eph} Step {step}: {loss_cell_type_prediction.item()}, "
                ...
                if step % finetune_args.get("val_evaluate", 10) == 0:
                    ...
                    logger.info(
                        f"[Validate] Stage {stage_idx + 1} Epoch {eph} Step {step}: {eval_loss:.4f}, "
                        f"Cell type accuracy: {eval_accuracy:.4f}, F1 score: {eval_f1_score:.4f}, "
                        f"LR: {optimizer.param_groups[0]['lr']:.6f}"
                    )
                    logger.info(
                        f"[Test] Stage {stage_idx + 1} Epoch {eph} Step {step}: {test_loss:.4f}, "
                    ...
                    if eval_f1_score > best_f1_score:
                        ...
                        logger.info(
                            f"[Best Model] Best validation F1 score: {best_f1_score:.4f} at stage {stage_idx + 1} "
                            f"epoch {eph} step {step}, test accuracy: {test_accuracy:.4f}, F1 score: {test_f1_score:.4f}"
                        )
                        torch.save(model.state_dict(), os.path.join(finetune_args["log_path"], "best_model.pt"))
                
                step += 1
            
            logger.info(f"Completed stage {stage_idx + 1} epoch {eph}")
        
        # Save model after each stage
        torch.save(model.state_dict(), os.path.join(finetune_args["log_path"], f"stage_{stage_idx + 1}_final.pt"))
...
def main_finetune():
    ...
    if args.progressive_unfreezing:
        cell_type_finetune_progressive(
            model,
            finetune_config,
            train_dataloader,
            val_dataloader,
