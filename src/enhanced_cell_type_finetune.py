"""
Enhanced fine-tuning script for ChromFound model with advanced training techniques
Implements all three improvement ideas:
...
def enhanced_cell_type_finetune(
        model,
        finetune_args,
        train_dataloader,
        val_dataloader,
    ...
    """
    Enhanced fine-tuning function implementing all three improvement ideas:
    1. Gradient clipping and dynamic learning rate adjustment
    2. Label smoothing
    3. Hierarchical learning rate fine-tuning (already implemented in optimizer setup)
    """
    ...
    for eph in range(finetune_args.get("epoch")):
        logger.info(f"Starting epoch {eph+1}/{finetune_args.get('epoch')}")
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # Step the learning rate scheduler (Idea 1)
            ...
            if step % finetune_args.get("loss_evaluate", 10) == 0:
                ...
                logger.info(
                    f"[Train] Epoch {eph+1}/{finetune_args.get('epoch')} - "
                    f"Step {step}: Loss={loss_cell_type_prediction.item():.4f}, "
            ...
            if step % finetune_args.get("val_evaluate", 10) == 0:
                ...
                logger.info(
                    f"[Validation] Epoch {eph+1}/{finetune_args.get('epoch')} - "
                    f"Step {step}: Loss={eval_loss:.4f}, "
                    f"Accuracy={eval_accuracy:.4f}, F1-Score={eval_f1_score:.4f}, "
                    f"LR={current_lr:.6f}"
                )
                logger.info(
                    f"[Test] Epoch {eph+1}/{finetune_args.get('epoch')} - "
                    f"Step {step}: Loss={test_loss:.4f}, "
                    f"Accuracy={test_accuracy:.4f}, F1-Score={test_f1_score:.4f}, "
                    f"LR={current_lr:.6f}"
                )
                ...
                if eval_f1_score > best_f1_score:
                    ...
                    logger.info(
                        f"New best validation F1-Score: {best_f1_score:.4f} at epoch {eph+1} step {step}, "
                        f"Test Accuracy: {test_accuracy:.4f}, Test F1-Score: {test_f1_score:.4f}"
                    )
                    torch.save(model.state_dict(), os.path.join(finetune_args["log_path"], "best_model.pt"))
            step += 1
            
        # Save model at the end of each epoch
        torch.save(model.state_dict(), os.path.join(finetune_args["log_path"], f"epoch_{eph+1}.pt"))
        logger.info(f"Completed epoch {eph+1}, model saved.")


def main_enhanced_finetune():
    parser = argparse.ArgumentParser(description="Enhanced fine-tuning for ChromFound with advanced techniques")
    ...
    parser.add_argument("--use_hierarchical_lr", action="store_true", default=True, help="use hierarchical learning rates")
    ...
    log_file_path = os.path.join(log_path, "enhanced_finetune.log")

    finetune_logger = setup_logging(log_file_path)

    finetune_logger.info('Enhanced Fine-tuning Logger is configured and ready.')
    ...
    # Setup optimizer with hierarchical learning rates (Idea 3)
    ...
    # Setup dynamic learning rate scheduler based on args (Idea 1)
    ...
    finetune_logger.info("Starting enhanced fine-tuning with all three improvement ideas:")
    finetune_logger.info("1. Gradient clipping and dynamic learning rate adjustment")
    finetune_logger.info("2. Label smoothing")
    finetune_logger.info("3. Hierarchical learning rate fine-tuning")
    
    enhanced_cell_type_finetune(
        model,
        finetune_config,
        train_dataloader,
        val_dataloader,
